#!/bin/sh
set -e

. "$(dirname "$0")/common_env.sh"
WORKSPACE_ROOT="$(workspace_root "$0")"
source_ros_setup
source_workspace_setup "$WORKSPACE_ROOT"
export WEBOTS_HOME="${WEBOTS_HOME:-/usr/local/webots}"
export PYTHONPATH="$WEBOTS_HOME/lib/controller/python${PYTHONPATH:+:$PYTHONPATH}"
export WEBOTS_CONTROLLER_HOST="${WEBOTS_CONTROLLER_HOST:-127.0.0.1}"
export WEBOTS_CONTROLLER_PORT="${WEBOTS_CONTROLLER_PORT:-1234}"
export WEBOTS_ROBOT_NAME="${WEBOTS_ROBOT_NAME:-Erebus_Bot}"
export WEBOTS_CONTROLLER_URL="${WEBOTS_CONTROLLER_URL:-tcp://$WEBOTS_CONTROLLER_HOST:$WEBOTS_CONTROLLER_PORT/$WEBOTS_ROBOT_NAME}"
CONTROLLER_PARAMS="${CONTROLLER_PARAMS:-$WORKSPACE_ROOT/install/fallitis_bringup/share/fallitis_bringup/config/stack.yaml}"
if [ ! -f "$CONTROLLER_PARAMS" ]; then
  CONTROLLER_PARAMS="$WORKSPACE_ROOT/install/share/fallitis_bringup/config/stack.yaml"
fi
if [ ! -f "$CONTROLLER_PARAMS" ]; then
  CONTROLLER_PARAMS="$WORKSPACE_ROOT/src/fallitis_bringup/config/stack.yaml"
fi
CONTROLLER_BIN="$WORKSPACE_ROOT/install/lib/fallitis_robot/webots_robot_node"
if [ ! -x "$CONTROLLER_BIN" ]; then
  CONTROLLER_BIN="$WORKSPACE_ROOT/install/fallitis_robot/lib/fallitis_robot/webots_robot_node"
fi
if [ ! -x "$CONTROLLER_BIN" ]; then
  echo "controller executable not found: $CONTROLLER_BIN" >&2
  echo "run: cd $WORKSPACE_ROOT && ./scripts/build_workspace.sh" >&2
  exit 1
fi
pkill -f '/fallitis_robot/webots_robot_node' 2>/dev/null || true
exec "$CONTROLLER_BIN" --ros-args --params-file "$CONTROLLER_PARAMS" "$@"
